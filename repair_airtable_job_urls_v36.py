"""Replace unreliable Google/Trabajo review URLs already saved in Airtable.

The script uses locally saved JSearch payloads to find a better employer, ATS,
LinkedIn/Indeed, or other stable publisher URL. If no stable alternative exists,
it clears Job URL instead of leaving a link that is misleading or unusable for
the reviewer. Contact data, Relevance, Status, and reviewer decisions are never
changed.
"""
...
from job_signal import (
    _is_google_jobs_url,
    _is_unreliable_review_url,
    select_job_url,
)
...
def _needs_repair(url: str) -> bool:
    return _is_google_jobs_url(url) or _is_unreliable_review_url(url)
...
def main() -> None:
    ...
    for record in records:
        ...
        if not _needs_repair(current_url):
            continue
        unreliable_urls_found += 1
        ...
        selected_is_usable = bool(
            selected
            and not _is_google_jobs_url(selected)
            and not _is_unreliable_review_url(selected)
        )
        patch: Dict = {
            "Job Signal Notes": _append_note(
                fields.get("Job Signal Notes"),
                f"v3.6_job_url_repair={reason}",
            ),
        }
    ...
    print(json.dumps({
        ...
        "unreliable_job_urls_found": unreliable_urls_found,
