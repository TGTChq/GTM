...
# Acquisition queries EVERY target title directly (one JSearch query per canonical
# role), so no target role depends on incidental appearance in the broad ATS /
# free-feed lanes for a deterministic acquisition path. Aliases remain match-only
# (never a separate query). Returned postings are still classified against the full
# catalog, so a query for one role also captures adjacent roles it happens to
# surface -- but every role additionally has its own intentional query.
#
# Cost: units = queries x NUM_PAGES (jsearch_scraper.estimate_query_units). With
# 118 queries this is 118 units at NUM_PAGES=1 -- within JSEARCH_MAX_ESTIMATED_
# UNITS_PER_RUN=150 -- so full coverage fits the existing quota without any
# increase (see PRODUCTION_DEPLOYMENT.md for the depth/quota tradeoff). Adaptive
# deepening then re-adds pages to the roles that actually yield, within budget.
DEFAULT_ACQUISITION_ROLES: Tuple[str, ...] = DEFAULT_SEARCH_ROLES
