...
class JobGate:
    ...
    def evaluate(self, job: Dict, *, fetch: Optional[bool] = None) -> GateDecision:
        ...
        if source.state == "SOURCE_TEMPORARILY_UNAVAILABLE":
            if _cross_source_minor_check(candidate, source):
                return GateDecision(
                    "job",
                    GateState.NEEDS_CHECK,
                    ReasonCode.NEEDS_CHECK_ATS_TEMPORARILY_UNAVAILABLE,
                    retryable=True,
                    next_action="write_review_and_continue_topup",
        ...
        if not duration.verified or duration.value != "open_ended":
            return _unknown("job", ReasonCode.UNVERIFIED_EMPLOYMENT_DURATION, bundle, source, secondary)
        ...
        return GateDecision(
            ...
            metadata={"source": source.to_dict(), "canonical_title": official_title},
        )
